...
export const ReturnAttachmentNames: React.FC<ReturnAttachmentProps> = ({
  array,
  projectName,
}) => {
  //NOTE: this is a temporary solution
  ...
  for (let i = 0; i < array.length; i++) {
    ...
    attachments.push(
      <div
        key={i + 1}
        className="round-corner-box"
        style={{ backgroundColor: attachmentColors[i], color: "#FFFFFF" }}
      >
        {projectName + attachmentTypes[i]}
      </div>,
    );
  }
